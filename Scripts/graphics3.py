...
class GraphicsObject:
    ...
    def draw(self, graphwin, a):
        ...
        self.id = self._draw(graphwin, self.config, a)
        graphwin.addItem(self)
        if graphwin.autoflush:
            _root.update()
        return self

    def animate(self, graphwin, a):
...
##                _root.update()
    ...
    def _draw(self, canvas, options, a):
...
class Image(GraphicsObject):
    ...
    def __init__(self, p, num, location, *pixmap):
        GraphicsObject.__init__(self, [])
        self.anchor = p.clone()
        self.imageId = location
#        Image.idCount = Image.idCount + 1
        frame = 0
        if len(pixmap) == 1: # file name provided
            while frame <= num:
                self.img = tk.PhotoImage(file=pixmap[0], master=_root, format='gif -index {}'.format(frame))  
    ...
    def _draw(self, canvas, options, a):
        p = self.anchor
        x,y = canvas.toScreen(p.x,p.y)
        return canvas.create_image(x,y,image=self.imageCache[a])
