...
def info(inverterAddress,solcast,preloadSet,dischargeDelay,setPreload,octopus,ssExport,freeTag,fStartHrs, fStartMins, fEndHrs, fEndMins, offset, win):
...
# preload switch
    text = Text(Point(180,30),'Set Preload')
    text._reconfig("font",("Arial",11,"normal"))
    text.draw(infoWin)

# delay switch
    text = Text(Point(260,30),'Set Delay')
    text._reconfig("font",("Arial",11,"normal"))
    text.draw(infoWin)

# octopus switch
    text = Text(Point(340,30),'Octopus')
    text._reconfig("font",("Arial",11,"normal"))
    text.draw(infoWin)

# plus Export switch
    text = Text(Point(415,30),'+ Export')
    text._reconfig("font",("Arial",11,"normal"))
    text.draw(infoWin)
...
#Preload switch
    if preloadSet == 'ON':
        switchOn(infoWin,180,60,'Blue')
    else:
        switchOff(infoWin,180,60)

#Discharge delay switch
    if dischargeDelay == 'ON':
        switchOn(infoWin,260,60,'Blue')
    else:
        switchOff(infoWin,260,60)

#Octopus switch
    if octopus == 'ON':
        switchOn(infoWin,340,60,'Green')
    else:
        switchOff(infoWin,340,60)

#plus Export switch
    if ssExport == 'ON':
        switchOn(infoWin,420,60,'Red')
    else:
        switchOff(infoWin,420,60)
...
# Write charging percentage
    ...
    # Mouse click on master window
            ...
            if m != None:
                infoWin.close()
                return solcast,preloadSet,dischargeDelay,setPreload,octopus,ssExport,freeTag,fStartHrs, fStartMins, fEndHrs, fEndMins, offset
    ...
    # plus Export switch pressed
                x = 420
                y = 60
                if (m.x > x-38) and (m.x < x+38) and (m.y > y-8) and (m.y < y+8):
                    ssExport = toggle(infoWin,x,y,'Red',ssExport)
