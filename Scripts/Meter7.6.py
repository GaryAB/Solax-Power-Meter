...
# clear pointers if relevant
  ...
  if ((int(chargepower/100) != int(oldcharge/100)) and (int(oldcharge/100) >= 0)) or (chargepower == 0) or (feedpower > chargepower):
   unfeedpointer = Line(Point(pointstartx[int(oldcharge/100)], pointstarty[int(oldcharge/100)]), Point(pointendx[int(oldcharge/100)], pointendy[int(oldcharge/100)]))
   unfeedpointer.setOutline("yellow")
   unfeedpointer.setWidth(8)
   unfeedpointer.draw(win)
   
  if ((int(feedpower/100) != int(oldfeedpower/100)) and (int(oldfeedpower/100) >= 0)) or (feedpower == 0) or (chargepower > feedpower):
...
# create pointer for charge power
  if (chargepower > 0) and (chargepower > feedpower):
    if chargepower > (scale*1000):
      chargepower = scale*1000

    mode = 'charge'
  
    chargepointer = Line(Point(pointstartx[int(chargepower/100)], pointstarty[int(chargepower/100)]), Point(pointendx[int(chargepower/100)], pointendy[int(chargepower/100)]))
