...
The Meter6.7.py and Meter7.7.py scripts have the ability to display error messages when a problem occurs.
...
Meter7.7.py has lost contact with the inverter
...
Remember that Meter7.7 cannot access the internet when connected directly to the SolaX inverter's access point.
...
The following errors only apply to Meter7.7:

>Inverter not found

Meter7.7 in DCHP mode has not been able to find the inverter on your local WiFi. This error may only be temporary. The script will wait for 5 minutes then try again.
...
For some reason Meter7.7 has been unable to automatically set the preload value. Use the SolaX Cloud or the SolaX app to set the preload manually.

>Discharge time not set

For some reason Meter7.7 has been unable to set the battery discharge start time. Use the SolaX cloud or the SolaX app to check the current setting and change it if necessary.

>Charge from grid disabled

Meter7.7 cannot set the preload as **Charge from grid** is disabled at the inverter. 
