...
The Meter7.8 script, when used on a Raspberry Pi, can be run either from the Graphical User Interface (GUI) as a script under Thony or as an embedded system that runs automatically every time the Raspberry Pi is rebooted. That requires some editing of the operating system on the Raspberry Pi, so read **Using the script as an embedded system**.

Meter 7.8.py has its settings in a separate file **Settings.py**.
