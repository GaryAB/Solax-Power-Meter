...
In this case you should leave the IP number in the script to the default '5.8.8.8'. If you have the newer Wifi module on the inverterm with 10 second updates, the IP number needs to be 192.168.10.10. 
...
In addition, Meter 7.8 has a facility which delays the start of the battery discharge phase on days of very poor solar yield in order that there is still power available from the batteries in the early evening, when the grid is under the greatest load.
