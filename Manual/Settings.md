...
Before you can use Meter7.9.py, you will first have to edit the settings for your solar installation.
...
**The next settings apply only to Meter7.9.py**

Meter7.9 can be used in two ways. You can either log into the inverter's WiFi access point (AP) directly, or you can contact the inverter via your house WiFi.
...
**Note** I have recently been told that later versions of the software on SolaX inverters no longer permit access to the API via your house WiFi. In this case, you **must** operate Meter 7.9 connected to the inverter's AP. Read **Instructions for using Meter7.9.py with the new SolaX Pocket Dongle v4.0** for an explanation on how to do this. 
...
In addition, Meter 7.9 has a facility which delays the start of the battery discharge phase on days of very poor solar yield so that there is still power available from the batteries in the early evening, when the grid is under the greatest load.
...
Note tnat the AUTO function is intended for the use of the script on a Raspberry Pi. As such it makes use of Linux system commands which may create an error on other operating systems.
...
The next settings are for the use of the Solcast module. Read the relevant instructions to set these.

If you have an electric vehicle, the EV function is available to prevent discharging your house batteries while your car is charging. Read **About the EV mode** to understand these settings.
