...
The time can be changed to your convenience. Do not however be tempted to reboot overnight as the script will assume that the backup file is out of date and ignore it when starting. 
...
Note that when running as an embedded system, the back up and log files are created in the your root directory rather than in the same directory as the script.
