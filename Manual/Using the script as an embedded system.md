...
The Meter 6.9 and Meter7.9 scripts when used on a Raspberry Pi can be used either from the Graphical User Interface (GUI) as a script under Thony, or it can be used as an embedded system which will run automatically every time that the Raspberry Pi is rebooted.
...
>@/usr/bin/python /home/yourname/folder/Meter7.9.py

Where **yourname** is the name of your home folder, **Meter7.9.py** is the current version of the script and **folder** is the folder where the script is placed.
...
The scripts contain a built-in watchdog timer. If any process takes longer than one minute, the Raspberry Pi will reboot. This should not affect the operation of the script, but if, for some reason, the script does not restart after the reboot, delete the temporary files; all those ending in **.json**, then reboot the Pi again. Note that this will lose your previous data, and the script will start from scratch.
...
For safety, reboot either before the start or after the end of your off-peak electricity period. 
